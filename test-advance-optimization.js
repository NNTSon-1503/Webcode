/**
 * TEST: Kiểm tra tác động của advance steps trên sync error
 */
...
class ChenReceiver {
    constructor(advance_steps = 50) {
        ...
        this.k_gain_y = 0.75;
        this.k_gain_z = 0.5;
        this.advance_steps = advance_steps;  // Same as master
    }
    ...
    syncWithMaster(receivedMaster) {
        for (let i = 0; i < this.advance_steps; i++) {
...
// ========================================================
// TEST: Compare 20 vs 50 vs 100 advance steps
// ========================================================
console.log("═══════════════════════════════════════════════════════════");
console.log("  COMPARISON: Advance Steps Impact on Sync Error");
console.log("═══════════════════════════════════════════════════════════\n");

for (const advance_steps of [20, 30, 50, 75, 100]) {
    const master = new ChenMaster();
    const receiver = new ChenReceiver(advance_steps);
    ...
    const timeMs = advance_steps * 0.001;

    console.log(`Advance Steps: ${advance_steps.toString().padStart(3)} (${timeMs.toFixed(1)}ms)`);
...
console.log("  - Advance steps = (send interval in milliseconds) / dt");
console.log("  - dt = 0.001, so: 50 steps = 50ms, 100 steps = 100ms");
console.log("  - Current system: Telemetry sent every 100ms → 100 steps");
console.log("  - Test options: 20(20ms), 30(30ms), 50(50ms), 75(75ms), 100(100ms)");
console.log("═══════════════════════════════════════════════════════════\n");
